...
type EnvironmentalData = {
  ...
  humidity?: string;
  attribution?: string;
  source?: { marine?: string; weather?: string };
};
...
function SectionTitle({ title, desc, action, onAction }: { title: string; desc?: string; action?: string; onAction?: () => void }) {
  return (
    <div className="mb-5 flex items-end justify-between gap-3">
      ...
      {action && <button onClick={onAction} className="hidden items-center gap-1 text-sm font-bold text-blue-600 md:flex">{action} <ChevronRight className="h-4 w-4" /></button>}
    </div>
  );
}
...
function PostCard({
  ...
  return (
    <motion.article initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} onClick={() => onOpen?.(post)} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md cursor-pointer">
      <div className="flex items-center gap-3 p-4">
        <div className="grid h-9 w-9 place-items-center overflow-hidden rounded-full bg-blue-50 text-lg">
          {post.avatar?.startsWith("http") ? <img loading="lazy" src={post.avatar} alt={`${post.user} 프로필`} className="h-full w-full object-cover" /> : post.avatar}
        </div>
        ...
        {canDelete && (
          <button onClick={(e) => { e.stopPropagation(); onDelete(post); }} className="rounded-xl p-2 text-slate-400 hover:bg-red-50 hover:text-red-600" title="내 게시글 삭제">
            <Trash2 className="h-4 w-4" />
          </button>
        )}
      </div>
      <img loading="lazy" src={post.img} alt={post.species} className={`${compact ? "h-44" : "h-52"} w-full bg-slate-100 object-contain`} />
      <div className="p-4">
        ...
        <div className="mt-4 flex items-center gap-4 text-sm text-slate-500">
          <button onClick={(e) => { e.stopPropagation(); onLike(post); }} className={`flex items-center gap-1 font-bold transition ${post.likedByMe ? "text-red-500" : "hover:text-red-500"}`}>
            <Heart className={`h-4 w-4 ${post.likedByMe ? "fill-current" : ""}`} />{post.likes}
          </button>
          <button onClick={(e) => { e.stopPropagation(); onComments(post); }} className="flex items-center gap-1 hover:text-blue-600">
            <MessageCircle className="h-4 w-4" />댓글 {post.comments}
          </button>
          <button onClick={(e) => e.stopPropagation()} className="ml-auto flex items-center gap-1 text-xs text-slate-400"><Flag className="h-3.5 w-3.5" />신고</button>
        </div>
      </div>
    </motion.article>
  );
...
function WeatherCard({ data, loading, onUseCurrentLocation }: { data: EnvironmentalData | null; loading?: boolean; onUseCurrentLocation?: () => void }) {
  const fallback: EnvironmentalData = {
    station: "부산 기장군 연안",
    observedAt: "대체 기준",
    waterTemp: "18.6°C",
    waveHeight: "0.4m",
  ...
  const env = data || fallback;
  return (
    <Card className="rounded-3xl border-0 bg-slate-900/92 text-white shadow-2xl backdrop-blur">
      <CardContent className="p-6">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-2xl font-black">오늘의 해황</h3>
            <p className="mt-3 flex items-center gap-1 text-sm text-slate-200"><MapPin className="h-4 w-4" /> {env.station || "가까운 관측소"}{env.stationDistanceKm ? ` · 약 ${env.stationDistanceKm}km` : ""}</p>
          </div>
          <p className="text-right text-xs text-slate-300">{loading ? "불러오는 중" : env.observedAt || "최신 기준"}</p>
        </div>
        <div className="grid grid-cols-2 gap-3 rounded-2xl border border-white/10 p-3">
          <div className="border-b border-r border-white/10 p-3"><p className="mb-2 text-xs text-slate-300">수온</p><p className="flex items-center gap-2 text-2xl font-black"><Thermometer className="h-6 w-6" />{env.waterTemp || "-"}</p></div>
          <div className="border-b border-white/10 p-3"><p className="mb-2 text-xs text-slate-300">파고</p><p className="flex items-center gap-2 text-2xl font-black"><Waves className="h-6 w-6" />{env.waveHeight || "-"}</p></div>
          <div className="border-r border-white/10 p-3"><p className="mb-2 text-xs text-slate-300">풍속</p><p className="flex items-center gap-2 text-2xl font-black"><Wind className="h-6 w-6" />{env.windSpeed || "-"}</p></div>
          <div className="p-3"><p className="mb-2 text-xs text-slate-300">기온/날씨</p><p className="flex items-center gap-2 text-xl font-black"><Droplets className="h-6 w-6" />{env.airTemp || "-"} · {env.weather || "-"}</p></div>
        </div>
        <div className="mt-3 grid gap-2 text-xs text-slate-300 md:grid-cols-2">
          <div className="rounded-xl bg-white/10 p-2">염분 {env.salinity || "확인 중"}</div>
          <div className="rounded-xl bg-white/10 p-2">조위 {env.tideLevel || "확인 중"}</div>
        </div>
        {onUseCurrentLocation && <Button onClick={onUseCurrentLocation} className="mt-4 w-full rounded-xl bg-white/10 py-5 font-bold text-white hover:bg-white/15">현재 위치 기준으로 새로고침 <Navigation className="ml-1 h-4 w-4" /></Button>}
        <p className="mt-3 text-[11px] leading-4 text-slate-400">{env.attribution || "기상 정보 제공: 기상청 · 해양 정보 제공: 국립해양조사원"}</p>
      </CardContent>
    </Card>
  );
}
...
export default function HoChaWebMVP() {
  ...
  const fetchPosts = async () => {
    if (!supabase) return;
    const { data, error } = await supabase.from("posts").select("*").order("created_at", { ascending: false }).range(0, 49);
    if (!error && data) {
  ...
  const fetchEnvironment = async (latitude?: number | null, longitude?: number | null, target: "home" | "upload" = "home") => {
    const lat = latitude ?? locationInfo.latitude ?? 35.244;
    const lon = longitude ?? locationInfo.longitude ?? 129.222;
    try {
      setEnvironmentLoading(true);
      const res = await fetch(`/api/environment?lat=${lat}&lon=${lon}`);
      const data = await res.json();
      if (target === "upload") setUploadEnvironment(data);
  ...
  const getCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        const env = await fetchEnvironment(latitude, longitude, "upload");
        setLocationMessage(`위치 저장 준비 완료 · 정확도 약 ${Math.round(accuracy)}m. 해황·기상 정보${env ? "도 함께 불러왔습니다" : "는 불러오지 못했습니다"}. 공개 지역명을 확인하는 중입니다...`);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from("posts").insert({
        ...
        water_temp: uploadEnvironment?.waterTemp,
        wave_height: uploadEnvironment?.waveHeight,
        wind_speed: uploadEnvironment?.windSpeed,
        salinity: uploadEnvironment?.salinity,
      });
  ...
  const AuthButtons = () => currentUser ? (
    <div className="flex items-center gap-2">
      <Button onClick={() => goToTab("profile")} variant="outline" className="rounded-xl border-blue-100 bg-blue-50 font-bold text-blue-700"><User className="mr-1 h-4 w-4" />내 프로필</Button>
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      {tab === "home" && <>
        <section className="relative overflow-hidden bg-slate-900"><div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1500375592092-40eb2168fd21?auto=format&fit=crop&w=1800&q=80')] bg-cover bg-center opacity-60" /><div className="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-950/60 to-slate-950/20" /><div className="relative mx-auto grid max-w-7xl gap-8 px-5 py-14 md:grid-cols-[1.25fr_0.75fr] md:items-center md:py-24"><motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}><h1 className="max-w-2xl text-5xl font-black leading-tight tracking-tight text-white md:text-7xl">연안의 모든 기록,<br />우리의 바다</h1><p className="mt-6 max-w-xl text-lg leading-8 text-slate-100">Ho-cha는 연안에서 만난 생물의 기록을 함께 나누는 공간입니다.</p><div className="mt-7 flex flex-wrap gap-3"><Button onClick={() => goToTab("upload")} className="rounded-xl bg-blue-600 px-6 py-6 text-base font-black hover:bg-blue-700"><Camera className="mr-2 h-5 w-5" />기록 올리기</Button><Button onClick={() => goToTab("feed")} variant="outline" className="rounded-xl border-white/30 bg-white/10 px-6 py-6 text-base font-black text-white hover:bg-white/20"><Eye className="mr-2 h-5 w-5" />둘러보기</Button></div><div className="mt-6 flex flex-wrap gap-2 text-sm text-white/90"><span className="rounded-full bg-white/15 px-3 py-1">사진 공유</span><span className="rounded-full bg-white/15 px-3 py-1">댓글 소통</span><span className="rounded-full bg-white/15 px-3 py-1">무료 GPS 기록</span><span className="rounded-full bg-white/15 px-3 py-1">도감 수집</span></div></motion.div><WeatherCard data={environmentData} loading={environmentLoading} onUseCurrentLocation={getCurrentLocation} /></div></section>
        <main className="mx-auto max-w-7xl px-5 py-8 md:py-10"><div className="grid gap-8 lg:grid-cols-[1fr_340px]"><div className="space-y-8"><section><SectionTitle title="오늘의 피드" desc={dbPostsLoaded && posts[0]?.isSample ? "아직 실제 기록이 없어 샘플을 보여줍니다." : "다른 사용자들이 올린 기록들입니다."} action="더보기" onAction={() => goToTab("feed")} /><div className="grid gap-4 md:grid-cols-3">{posts.slice(0, 3).map((post) => <PostCard key={post.id} post={post} compact currentUserId={currentUser?.id} onComments={openComments} onLike={toggleLike} onDelete={deletePost} onOpen={openPostDetail} />)}</div></section><section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><SectionTitle title="최근 기록" desc="새로 올라온 기록이 이곳에 표시됩니다." action="더보기" onAction={() => goToTab("feed")} /><div className="grid gap-4 md:grid-cols-3">{posts.slice(0, 3).map((post) => <PostCard key={`recent-${post.id}`} post={post} compact currentUserId={currentUser?.id} onComments={openComments} onLike={toggleLike} onDelete={deletePost} onOpen={openPostDetail} />)}</div></section><section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><SectionTitle title="오늘의 생물" desc="최근 기록에서 가장 많이 보인 생물을 자동으로 보여줍니다." action="도감 보기" onAction={() => goToTab("book")} />{todaySpecies ? <div className="grid gap-4 md:grid-cols-[180px_1fr]"><img loading="lazy" src={todaySpecies.recentPost.img} alt={todaySpecies.name} className="h-40 w-full rounded-2xl object-cover" /><div className="flex flex-col justify-center"><div className="flex flex-wrap items-center gap-2"><p className="text-3xl font-black text-slate-900">{todaySpecies.name}</p>{todaySpecies.meta && <span className="rounded-full bg-purple-50 px-3 py-1 text-xs font-bold text-purple-700">희귀도 {todaySpecies.meta.rarity}</span>}</div><p className="mt-2 text-sm leading-6 text-slate-600">최근 기록 {todaySpecies.count}건 · 주요 지역 {Array.from(todaySpecies.locations).slice(0, 3).join(", ") || "지역 미입력"}</p><p className="mt-2 text-sm leading-6 text-slate-500">{todaySpecies.meta?.note || "표준종 연결 전 기록입니다. 도감과 동정 요청을 통해 더 정확한 기록으로 정리할 수 있습니다."}</p><Button onClick={() => goToTab("book")} variant="outline" className="mt-4 w-fit rounded-xl bg-slate-50 font-bold">도감에서 보기 <ChevronRight className="ml-1 h-4 w-4" /></Button></div></div> : <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center"><Fish className="mx-auto mb-2 h-10 w-10 text-slate-400" /><p className="font-bold text-slate-700">아직 오늘의 생물을 뽑을 실제 기록이 없습니다.</p><p className="mt-1 text-sm text-slate-500">첫 기록을 올리면 이곳에 표시됩니다.</p></div>}</section><section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><SectionTitle title="이번 주 출현 요약" desc="실제 기록을 바탕으로 자동 계산됩니다." action="통계 보기" onAction={() => goToTab("stats")} /><div className="grid gap-3 md:grid-cols-3"><div className="rounded-2xl bg-blue-50 p-4"><p className="text-xs font-bold text-blue-600">총 기록</p><p className="mt-1 text-2xl font-black text-slate-900">{weeklyStats.total}건</p></div><div className="rounded-2xl bg-emerald-50 p-4"><p className="text-xs font-bold text-emerald-700">위치 기록</p><p className="mt-1 text-2xl font-black text-slate-900">{weeklyStats.locatedCount}건</p></div><div className="rounded-2xl bg-amber-50 p-4"><p className="text-xs font-bold text-amber-700">동정 요청</p><p className="mt-1 text-2xl font-black text-slate-900">{weeklyStats.requestCount}건</p></div></div></section></div><aside className="space-y-5"><Card className="rounded-2xl border-slate-200 bg-white shadow-sm"><CardContent className="p-5"><h3 className="mb-4 text-xl font-black">내 활동 요약</h3>{currentUser ? <><p className="text-sm text-slate-500">내가 올린 기록</p><p className="mt-2 text-4xl font-black text-blue-600">{currentUserPosts.length}<span className="text-xl">건</span></p><p className="mt-3 text-sm text-slate-500">기록 생물 {mySpeciesCount}종</p><Button onClick={() => goToTab("profile")} variant="outline" className="mt-5 w-full rounded-xl bg-slate-50 font-bold">내 프로필 보기 <ChevronRight className="ml-1 h-4 w-4" /></Button></> : <><p className="text-sm leading-6 text-slate-600">로그인하면 내 기록과 도감 현황을 따로 볼 수 있습니다.</p><Button onClick={() => openAuth("login")} className="mt-5 w-full rounded-xl bg-blue-600 font-bold">로그인</Button></>}</CardContent></Card><Card className="rounded-2xl border-slate-200 bg-white shadow-sm"><CardContent className="p-5"><h3 className="text-xl font-black">출현 모니터링</h3><div className="mt-4 grid h-44 place-items-center rounded-xl bg-gradient-to-br from-blue-50 to-slate-100 text-center"><div><Map className="mx-auto mb-2 h-10 w-10 text-blue-500" /><p className="text-sm font-bold text-slate-700">무료 GPS 좌표 저장</p><p className="mt-1 text-xs text-slate-500">공개 화면에는 권역만 표시</p></div></div><Button onClick={() => goToTab("data")} variant="outline" className="mt-5 w-full rounded-xl bg-slate-50 font-bold">자세히 보기 <ChevronRight className="ml-1 h-4 w-4" /></Button></CardContent></Card></aside></div><section className="mt-10 rounded-3xl bg-blue-50 p-6 md:p-8"><div className="grid gap-6 md:grid-cols-5 md:items-start"><div><h3 className="text-2xl font-black">Ho-cha 란?</h3><p className="mt-3 text-sm leading-6 text-slate-700">“호차”는 연안에서 뜰채를 들던 순간에 나는 소리로, 친구들과 생물을 탐사하던 기억에서 시작되었습니다. Ho-cha는 그 기억과 바다의 기록을 담는 공간입니다.</p></div>{[[Camera, "기록하고 공유하기", "사진과 함께 생물 기록을 남겨주세요."], [MapPin, "위치 저장", "무료 GPS로 좌표를 저장하고 공개는 권역만 표시합니다."], [BookOpen, "도감 채우기", "다양한 생물을 발견하고 나만의 도감을 완성하세요."], [Leaf, "바다를 지키기", "정확한 기록이 깨끗한 바다를 만드는 첫걸음입니다."]].map(([Icon, title, desc]: any) => <div key={title} className="text-center"><div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-white text-blue-600 shadow-sm"><Icon className="h-6 w-6" /></div><p className="mt-3 font-black">{title}</p><p className="mt-2 text-sm leading-6 text-slate-600">{desc}</p></div>)}</div></section></main>
      </>}

      {tab !== "home" && <main className="mx-auto max-w-7xl px-5 py-8 md:py-10">
        {tab === "postDetail" && activePost && <section className="mx-auto max-w-4xl">
          <button onClick={() => goToTab("feed")} className="mb-4 inline-flex items-center gap-1 rounded-xl bg-white px-4 py-2 text-sm font-bold text-slate-600 shadow-sm ring-1 ring-slate-200 hover:text-blue-600">← 피드로 돌아가기</button>
          <Card className="overflow-hidden rounded-3xl border-slate-200 bg-white shadow-sm">
            <div className="flex items-center gap-3 p-5">
              <div className="grid h-12 w-12 place-items-center overflow-hidden rounded-full bg-blue-50 text-xl">{activePost.avatar?.startsWith("http") ? <img loading="lazy" src={activePost.avatar} alt={`${activePost.user} 프로필`} className="h-full w-full object-cover" /> : activePost.avatar}</div>
            ...
            <img loading="lazy" src={activePost.img} alt={activePost.species} className="max-h-[720px] w-full bg-slate-100 object-contain" />
                      ...
                      {comments.length === 0 ? <p className="text-sm text-slate-500">아직 댓글이 없습니다. 첫 댓글을 남겨보세요.</p> : comments.map((c) => (
                        <div key={c.id} className="rounded-xl border border-slate-100 bg-slate-50 p-3 text-sm text-slate-700">
                          <div className="mb-2 flex items-center gap-2">
                            <div className="grid h-8 w-8 place-items-center overflow-hidden rounded-full bg-blue-50 text-xs">{c.avatar?.startsWith("http") ? <img loading="lazy" src={c.avatar} alt={`${c.user || "사용자"} 프로필`} className="h-full w-full object-cover" /> : c.avatar}</div>
                            <div>
